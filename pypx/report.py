...
class Report(Base):
            ...
            for t in [
                ...
                for k, v in t.items():
                    if k in self.arg.keys():
                        if len(self.arg[k]):
                            self.d_reportTags[v[0]][v[1]] = self.arg[k].split(",")

    def header_studySanitize(self):
        self.d_reportTags["header"]["study"] = [
            k
            for k in self.d_reportTags["header"]["study"]
            if k in self.arg["reportData"]["command"]
        ]
        if (
            "Modality" not in self.arg["reportData"]["command"]
            or "QueryRetrieveLevel=STUDY" in self.arg["reportData"]["command"]
        ):
    ...
    def report_generate(self):
                    ...
                    if "then" in self.arg["reportData"]:
                        ...
                        try:
                            ...
                            str_value = self.seriesStatus_print(
                                ...
                                status=d_series,
                                SeriesDescription=d_DICOMfields["SeriesDescription"][
                                    "value"
                                ],
                            )
                        except:
                            str_value = (
                                "Status error for studyIndex %d / seriesIndex %d."
                            )
            return str_value

        def block_build(
            l_DICOMtag, l_blockFields, l_blockTable, str_reportBlock, d_block
        ):
            """
            Essentially create a text/table of rows each of 2 columns.
            """

            def tableRow_add2Col(
                str_left, str_right, leftColWidth=30, rightColWidth=50
            ):
                """
                ...
                return [
                    CheaderField + f"{str_left:<30}" + Colors.NO_COLOUR,
                    CheaderValue + f"{str_right:<50}" + Colors.NO_COLOUR,
                ]

            def row_add2Col(str_left, str_right, leftColWidth=30, rightColWidth=50):
                """
            ...
            for str_tag in l_blockFields:
                l_blockTable.append(
                    tableRow_add2Col(str_tag, DICOMtag_lookup(l_DICOMtag, str_tag))
                )
                str_reportBlock += row_add2Col(
                    str_tag, DICOMtag_lookup(l_DICOMtag, str_tag)
                )
        ...
        def header_generate(study):
            ...
            for k in self.d_reportTags["header"]:
                ...
                l_headerTable, str_reportHeader, d_headerContents = block_build(
                    analyze, l_tags, l_headerTable, str_reportHeader, d_headerContents
                )
            ...
            for k in d_bodyFields.keys():
                ...
                if k == "series" and "series" in study:
                    ...
                    for series in l_series:
                        # pudb.set_trace()
                        l_bodyTable, str_reportBody, d_bodyContents = block_build(
                            series, l_tags, l_bodyTable, str_reportBody, d_bodyContents
                        )
                        ...
                        l_suidTable, str_reportSUID, d_seriesUID = block_build(
                            series,
                            l_seriesUIDtag,
                            l_suidTable,
                            str_reportSUID,
        ...
        for study in self.arg["reportData"]["data"]:
            ...
            # Generate the header
            d_tabular["header"], d_rawText["header"], d_json["header"] = (
        ...
        return {"tabular": l_tabularHits, "rawText": l_rawTextHits, "json": l_jsonHits}
    ...
    def toCSV(self, **kwargs):
        ...
        def padListToMax(l_lv: list, str_header: str = "") -> tuple:
            ...
            if len(str_header) + 2 > width:
                width = len(str_header) + 2
        ...
        def checkAndPrettify() -> bool:
            ...
            if self.arg["csvPrettify"]:
                ...
                if len(self.arg["reportBodySeriesTags"]):
                    ...
                    for tag in l_tag:
                        l_padded, width = padListToMax(
                            [k[tag] for k in ld_seriesDesc], tag
                        )
                    ...
                    l_padded, width = padListToMax(
                        [k["SeriesDescription"] for k in ld_seriesDesc],
                        "SeriesDescription",
                    )
                    for i in range(0, len(ld_seriesDesc)):
                    ...
                    if str_colToJustify in ld_seriesUID[0]:
                        ...
                        try:
                            l_w = [
                                "%05d" % int(f[str_colToJustify]) for f in ld_seriesUID
                            ]
                        except:
        ...
        def checkAndSummarize() -> bool:
            ...
            if self.arg["csvSummarize"]:
                b_status = True
                l_descriptionSummary = ["Study contains %d series" % len(ld_seriesDesc)]
                l_instanceSummary = [
                    "All series contain %d images\n"
                    % sumColElements(
                        [
                            ld_seriesUID[i][str_colToSum]
                            for i in range(0, len(ld_seriesUID))
                        ]
                    )
                ]
                ld_seriesDesc = []
                ld_seriesDesc.append(
                    {"%s" % "SeriesDescription": "%s" % l_descriptionSummary[0]}
                )
        ...
        def colHeaders_generate() -> str:
            ...
            if len(self.arg["reportBodySeriesTags"]):
                ...
                str_bodyLabels = "%s%s%s" % (
                    "SeriesDescription".center(
                        len(ld_seriesDesc[0]["SeriesDescription"])
                    ),
                    self.arg["csvSeparator"],
                    "NumberOfSeriesRelatedInstances".center(
                        len(ld_seriesUID[0]["NumberOfSeriesRelatedInstances"])
                    ),
                )
            return str_headerLabels + self.arg["csvSeparator"] + str_bodyLabels + "\n"
        ...
        for d_study in self.d_report["json"]:
            ...
            if self.arg["csvPrintHeaders"]:
                if b_prettifyDo:
                    str_csvReport += str_tableTop
                str_csvReport += str_columnHeaders
                if b_prettifyDo:
                    str_csvReport += str_tableMiddle
            else:
                if b_prettifyDo:
                    str_csvReport += str_tableTop
            ...
            for ds, dn in zip(ld_seriesDesc, ld_seriesUID):
                if len(self.arg["reportBodySeriesTags"]):
                    ...
                    str_seriesVals = ""
                    for tag in l_tags:
                        str_seriesVals += "%s%s" % (ds[tag], self.arg["csvSeparator"])
    ...
    def seriesRetrieve_print(self, **kwargs) -> str:
        ...
        if studyIndex >= 0:
            if seriesIndex >= 0:
                str_seriesInstances = "%03d" % int(
                    self.report_getBodyField(
                        studyIndex, seriesIndex, "NumberOfSeriesRelatedInstances"
                    )
                )
                str_seriesDescription = self.report_getBodyField(
                    studyIndex, seriesIndex, "SeriesDescription"
                )
                str_seriesNumber = self.report_getBodyField(
                    studyIndex, seriesIndex, "SeriesNumber"
                )
                if len(str_seriesNumber):
                    str_seriesNumber = " [ %04d ] " % int(str_seriesNumber)
                str_line = "Requesting " + str_seriesInstances + " images for "
        ...
        if studyIndex >= 0:
            if seriesIndex >= 0:
                ...
                if not len(str_seriesInstanceUID):
                    try:
                        str_seriesInstanceUID = self.report_getBodyField(
                            studyIndex, seriesIndex, "SeriesInstanceUID"
                        )
                    except:
                        str_seriesInstanceUID = ""
                if d_status["status"]:
                    str_line = "  PACS:%s     OXIDCM:%s        CUBE:%s  │ " % (
                        str_requestedCount,
                        str_pushedCount,
                        str_registeredCount,
                    )
    ...
    def seriesRegister_print(self, **kwargs) -> str:
        ...
        if studyIndex >= 0:
            if seriesIndex >= 0:
                ...
                str_line = "%-50s" % str_line
                str_line = (
                    "Registering [ %s ]: " % (str_receivedCount)
                    + str_line
                    + str_seriesInstanceUID
                )
            else:
    ...
    def report_print(self):
        ...
        if str_field == "tabular" or str_field == "rawText":
            ...
            print(json.dumps(self.d_report["json"], indent=4))
    ...
    def run(self, opt={}):
            ...
            if len(self.arg["reportDataFile"]):
                try:
                    ...
                    print(
                        ...
                        % (self.arg["reportDataFile"], "%s" % e)
                    )
                    b_status = False

        if "status" in self.arg["reportData"]:
            if (
                not self.arg["reportData"]["status"]
                or "error" in self.arg["reportData"]["status"]
            ):
                self.log(
                    "An error has been flagged in the report payload.", comms="error"
                )
                self.log(
        ...
        if b_status:
            ...
            if len(self.printReport):
                if (
                    self.printReport in self.d_report.keys()
                    or self.printReport == "csv"
                ):
                    self.report_print()
